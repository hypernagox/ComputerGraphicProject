...
void main()
{
   ...
   FragColor = min(vec4(result,1.f) * vec4(abs(Frag),1.f) , vec4(1.0, 1.0, 1.0, 1.0));
...
vec3 CalculateSpotLight(SpotLightData light, vec3 normal, vec3 fragPos, vec3 viewPos)
{
    ...
    float attenuation = CalculateAttenuation(distance, light.constant, light.linear, light.quadratic) * 200;
...
vec3 CalculateDirectionalLight(DirectionalLightData light, vec3 normal, vec3 fragPos,vec3 viewpos)
{
    ...
    vec3 ambient = light.lData.ambient;
    ...
    vec3 diffuse = light.lData.diffuse * diff ;

   
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), 0);
    vec3 specular = light.lData.specular * spec ;
